...
{   
    public function finish(){
        ...
        if(\Auth::guest()){return response(['success' => false, 'mess' => 'Authorize' ]);}

        $user = Auth::user();

        $games_on = 0;
        if(\Cache::has('coinGame.user.'. $user->id.'start')) $games_on = \Cache::get('coinGame.user.'. $user->id.'start');
        if($games_on == 0) return response(['success' => false, 'mess' => 'An unknown error has occurred']);
        ...
        if($game->step < 1){
            return response(['success' => false, 'mess' => 'You have not passed any level' ]);
        }
    ...
    public function play(Request $r){
        ...
        if($games_on == 0){
            return response(['success' => false, 'mess' => 'An unknown error has occurred']);
        }
        // $game = Coin::where('user_id', $user->id)->first();
