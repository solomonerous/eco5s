...
const server = require("https").createServer({
    key: fs.readFileSync('/etc/letsencrypt/live/eco5s.com/privkey.pem'),
    cert: fs.readFileSync('/etc/letsencrypt/live/eco5s.com/fullchain.pem')
}),
io = require("socket.io")(server, {
    cors: {
        origin: "https://eco5s.com",
        methods: ["GET", "POST"]
    }
});
...
function stretchArray(arr, limit) {
  // Check if the array is empty or the limit is less than the array length
  if (!arr.length || limit < arr.length) {
    throw new Error('Failed to stretch the array.');
  }

  // Calculate how many times each element will be replicated (integer part)
  const integer = Math.floor(limit / arr.length);

  // Calculate the remainder (crumbs) for uneven distribution
  let crumbs = limit % arr.length;

  // Create a new array by replicating each element according to the calculated counts
  const newArr = Array.from(arr, function(item, index) {
    // Determine the number of times to replicate each element
    const repeater = integer + (crumbs > 0 ? 1 : 0);

    // Decrease the number of remaining crumbs
    crumbs--;

    // Replicate the element and return
  ...
  // Alternatively, you can use the flat method:
  // return newArr.flat();
}
